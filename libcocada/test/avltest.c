...
void test_avl_remv(CuTest *tc)
{
	...
	CuAssert(tc, "Attempt to delete non-existent element should return false",
	         ! avl_remv_int(tree, 2*half_univ, &res));
	DEBUG_EXEC(avl_print(tree, stderr, print_int));
	DEBUG("\n\n\n");
	// remove leaf node
	CuAssert(tc, "Failed deletion", avl_remv_int(tree, 2, &res));
	DEBUG_EXEC(avl_print(tree, stderr, print_int));
	DEBUG("\n\n\n");
	CuAssert(tc, "Failed deletion", avl_remv_int(tree, 4, &res));
	DEBUG_EXEC(avl_print(tree, stderr, print_int));
	DEBUG("\n\n\n");
	CuAssert(tc, "Failed deletion", avl_remv_int(tree, 6, &res));
	DEBUG_EXEC(avl_print(tree, stderr, print_int));
	DEBUG("\n\n\n");
	CuAssert(tc, "Failed deletion", avl_remv_int(tree, 13, &res));
	DEBUG_EXEC(avl_print(tree, stderr, print_int));
	DEBUG("\n\n\n");
	CuAssert(tc, "Failed deletion", avl_remv_int(tree, 17, &res));
	DEBUG_EXEC(avl_print(tree, stderr, print_int));
	DEBUG("\n\n\n");
	CuAssert(tc, "Failed deletion", avl_remv_int(tree, 19, &res));
	DEBUG_EXEC(avl_print(tree, stderr, print_int));
	DEBUG("\n\n\n");
	DESTROY_FLAT(tree, avl);
	CuAssert(tc, "Memory leak.", memdbg_is_empty());
	...
	obj_t probe;
	obj_t *dest;
	probe.key = 2*half_univ;
	CuAssert(tc, "Attempt to delete non-existent element should return false",
	         ! avl_remv_rawptr(tree, &probe, &dest));
	DEBUG_EXEC(avl_print(tree, stderr, print_obj_t_ref));
	DEBUG("\n\n\n");
	// remove leaf node
	probe.key = 2;
	CuAssert(tc, "Failed deletion", avl_remv_rawptr(tree, &probe, &dest));
	DEBUG_EXEC(avl_print(tree, stderr, print_obj_t_ref));
