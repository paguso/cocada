...
staticlib: $(STATIC_LIB_DIR) $(DEP_STLIBS) build $(STLIB)
...
DEP_SHARED_LIBS := $(patsubst %,%.so,$(DEP_LIBS))
...
#%.so: lib%.so 
#	@echo "(cocadabio) ln -rs $(wildcard $(SHARED_LIB_DIR)/$<.*) $(SHARED_LIB_DIR)/$<"
#	@echo "(cocadabio) ln -rs (wildcard $(SHARED_LIB_DIR)/$<.*) $(SHARED_LIB_DIR)/$<"
#	ls -lah $(SHARED_LIB_DIR)
#	ln -rs $(wildcard $(SHARED_LIB_DIR)/$<.*) $(SHARED_LIB_DIR)/$<

DEP_LIBS_SONAMES := $(patsubst %,$(SHARED_LIB_DIR)/lib%.so,$(DEP_LIBS))
...
sharedlib: clean-sharedlib $(DEP_SHARED_LIBS) $(DEP_SHARED_LIBS_SONAMES) rebuild make-sharedlib
	...
	@echo "DEP_SHARED_LIBS = [$(DEP_SHARED_LIBS)]"
	

install-sharedlib: sharedlib
	@echo Installing headers to $(INCLUDE_DIR)
	$(MKDIR) $(INCLUDE_DIR)
	cp $(SHARED_LIB_DIR)/include/*.h $(INCLUDE_DIR)
