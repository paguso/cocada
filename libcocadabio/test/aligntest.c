...
void test_affine_global_align(CuTest *tc)
{
	...
	int gap_open = 0;
	int gap_ext = 1;
	int gotoh_cost = affine_global_align(qry, strlen(qry), tgt, strlen(tgt),
	                                     gap_open, gap_ext, unit_subst, NULL);
	int align_cost = affine_global_align(qry, strlen(qry), tgt, strlen(tgt),
	                                     gap_open, gap_ext, unit_subst, cigar);
...
void test_fprintf_alignment(CuTest *tc)
{
	...
	fprintf_alignment(stdout, qry, 0, strlen(qry), tgt, 12, 47, cigar,
	                  strlen(cigar));
}
