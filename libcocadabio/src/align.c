...
static int read_number_(const char *cigar, usize pos, int *nb)
{
                        ...
                        usize tgt_len, StrBuf *cigar)
{
	...
	for (usize i = 0; i <= qry_len; i++) {
		C[i][0] = i;
	}
	for (usize j = 0; j <= tgt_len; j++) {
		C[0][j] = j;
	}
	for (usize i = 1; i <= qry_len; i++) {
		for (usize j = 1; j <= tgt_len; j++) {
                             ...
                             usize tgt_len, int gap_open, int gap_ext, SubstCostFunc subst)
{
	...
	for (usize i = 1; i < m; i++) {
		...
		//II[i][0] = t + gap_open;
		C_iminus1_jminus1 =
		    C_iminus1_j;// has to be set to C[i-1][0] so when it enters j=1 loop, this will be C[i-1][j-1];
		for (usize j = 1; j < n; j++) {
			...
			//assert(D[j] == DD[i][j]);
			C_iminus1_j =
			    C[j]; // C[j] contains C[i-1][j] so in the next j-iteration C_iminus1_j will contain C[i-1][j-1]
                          ...
                          SubstCostFunc subst, int *C, int *D)
{
	...
	for (usize i = 1; i < m; i++) {
		//C_iminus1_j = C[0]; // C_iminus1_j := C[i-1][0]
		C_iminus1_jminus1 =
		    C[0];//C_iminus1_j;// has to be set to C[i-1][0] so when it enters j=1 loop, this will be C[i-1][j-1];
		t += gap_ext;
		C[0] = t;
		D[0] = t;
		I_i_j = t + gap_open;
                         ...
                         StrBuf *cigar, int *C1, int *D1, int *C2, int *D2)
{
	...
	if (n == 0) {
		...
		usize mid_qry = (from_qry + to_qry) / 2;
		usize top_len = mid_qry - from_qry;
		...
		if (join == CAT_JOIN) {
			int tc = aff_slice_aln(qry, from_qry, mid_qry, tgt, from_tgt, mid_tgt, gap_open,
			                       gap_open_begin, gap_open, gap_ext, subst, cigar, C1, D1, C2, D2);
			int bc = aff_slice_aln(qry, mid_qry, to_qry, tgt, mid_tgt, to_tgt, gap_open,
			                       gap_open, gap_open_end, gap_ext, subst, cigar, C1, D1, C2, D2);
			assert (min_cost == tc + bc);
		}
		else if (join == DEL_JOIN) {
			int tc = aff_slice_aln(qry, from_qry, mid_qry - 1, tgt, from_tgt, mid_tgt,
			                       gap_open, gap_open_begin, 0, gap_ext, subst, cigar, C1, D1, C2, D2);
			sbprintf(cigar, "2D");
			int bc = aff_slice_aln(qry, mid_qry + 1, to_qry, tgt, mid_tgt, to_tgt, gap_open,
			                       0, gap_open_end, gap_ext, subst, cigar, C1, D1, C2, D2);
...
{
	usize qry_len = qry_to - qry_from;
	usize tgt_len = tgt_to - tgt_from;
	qry = &qry[qry_from];
	tgt = &tgt[tgt_from];
	StrBuf *qry_line = strbuf_new();
	StrBuf *tgt_line = strbuf_new();
	StrBuf *cig_line = strbuf_new();
	usize tpos = 0, qpos = 0, cpos = 0, prev_cpos = 0;
	...
	fprintf(out, "%s\n%s\n%s\n", strbuf_as_str(tgt_line),  strbuf_as_str(qry_line),
	        strbuf_as_str(cig_line));
